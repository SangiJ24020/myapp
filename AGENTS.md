...
### ⚡ 高速化・データキャッシュ仕様
- アプリ初期起動の高速化（体感0秒化）のため、`mode: 'getInitialData'` でタスク・プロジェクト・統計レポートを1往復でまとめて返却する。
- フロントエンドでは `localStorage` を活用した Stale-While-Revalidate パターンを採用し、起動時に前回のキャッシュデータを0.1秒で即時描画したうえで、バックグラウンドでGASから最新データを取得・同期する。
- GAS側の `initSheets()` は `CacheService` を用いて不要な毎リクエスト走査をスキップする。
